...
public abstract class DeviceEx
{
    ...
    public string ConnectedAppType { get; set; } = string.Empty;
    ...
    {
        var handler = physicalStatusChanged;
        if (handler != null)
        {
            Delegate[] invocationList = handler.GetInvocationList();
            for (int i = 0; i < invocationList.Length; i++)
            {
                ((EventHandler<DevicePhysicalStateEx>)invocationList[i]).Invoke(sender, e);
            }
        }
    }
    ...
    {
        var handler = softStatusChanged;
        if (handler != null)
        {
            Delegate[] invocationList = handler.GetInvocationList();
            for (int i = 0; i < invocationList.Length; i++)
            {
                ((EventHandler<DeviceSoftStateEx>)invocationList[i]).Invoke(sender, e);
            }
        }
    }
