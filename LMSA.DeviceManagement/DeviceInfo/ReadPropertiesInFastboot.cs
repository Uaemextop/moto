...
public class ReadPropertiesInFastboot
{
    ...
    private Dictionary<string, string> ReadAll()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                LogHelper.LogInstance.Info($"Testlog-->Fastboot getvar all Other Resp:{list}");
            }
            foreach (string item in list)
            {
                ...
                {
                    dictionary.Add(value, value2?.Trim() ?? string.Empty);
                }
            }
        }
        catch (Exception ex)
    ...
    private void ReadSimConfig()
    {
        ...
        do
        {
            list = ProcessRunner.ProcessList(Configurations.FastbootPath, text, 20000) ?? new List<string>();
