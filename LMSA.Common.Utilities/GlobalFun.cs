...
public class GlobalFun
{
	...
	// WPF-dependent method - requires System.Windows.Media.Imaging
	// public static BitmapImage LoadBitmap(string imageUri) { throw new NotImplementedException("WPF required"); }

	// WPF-dependent method - requires System.Windows.Media.Imaging
	// public static BitmapImage ConvertBitmap(Bitmap bitmap) { throw new NotImplementedException("WPF required"); }
	...
	public static bool CheckServerIsRunning(string serviceName)
	{
		...
		ServiceController val = ((IEnumerable<ServiceController>)ServiceController.GetServices()).FirstOrDefault((Func<ServiceController, bool>)((ServiceController n) => n.ServiceName.Equals(serviceName, StringComparison.CurrentCultureIgnoreCase)));
		if (val == null)
		{
			return false;
		}
		...
		if (systemType == null)
		{
			...
			try
			{
				...
				try
				{
					...
					{
						PropertyData? obj = ((IEnumerable<PropertyData>)((IEnumerable)enumerator.Current.Properties).Cast<PropertyData>().ToList()).FirstOrDefault((Func<PropertyData, bool>)((PropertyData n) => n.Name == "SystemType"));
