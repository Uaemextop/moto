...
public class WarrantyService : ApiService
{
    ...
    public static int GetBuyedWarranty(string region, string imei)
    {
        ...
        if (RequestPostFormData("https://api-pre-mds-us.lenovo.com/auth/oauth/token", out var result, new Dictionary<string, string> { { key, value } }, WarrantyResources.Name, WarrantyResources.Code) && !string.IsNullOrEmpty(result))
        {
    ...
    private static bool RequestPostFormData(string url, out string result, Dictionary<string, string> formdata, string username, string password)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                try
                {
                    val2.Headers.Add("ContentType", "multipart/form-data;charset=UTF-8");
                    if (formdata != null && formdata.Count > 0)
                    {
                        foreach (KeyValuePair<string, string> formdatum in formdata)
                        {
                            val2.Add(new StringContent(formdatum.Value ?? ""), formdatum.Key);
                        }
                    }
                    ...
                    Task<HttpResponseMessage> task = val.PostAsync(url, val2);
                    task.Wait();
