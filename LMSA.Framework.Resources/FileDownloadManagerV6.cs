...
public class FileDownloadManagerV6 : IFileDownload
{
	...
	public Tuple<List<DownloadInfo>, List<DownloadInfo>, List<string>> Load()
	{
		try
		{
			Tuple<List<DownloadInfo>, List<string>> tuple = Rsd.Instance.InitDownloadingResources();
			Tuple<List<DownloadInfo>, List<string>> tuple2 = Rsd.Instance.InitDownloadedResources();
			List<string> list = new List<string>(tuple.Item2);
			list.AddRange(tuple2.Item2);
			list = list.Distinct().ToList();
			tuple2.Item1.ForEach(delegate(DownloadInfo n)
			{
				DownloadedTasks.AddOrUpdate(n.FileUrl, n);
			});
			return new Tuple<List<DownloadInfo>, List<DownloadInfo>, List<string>>(tuple.Item1, tuple2.Item1, list);
		}
		catch (Exception)
		{
			return null;
	...
	private void FireStatusChanged(DownloadInfo e)
	{
		...
		if (eventHandler != null)
		{
			...
			{
				((EventHandler<RemoteDownloadStatusEventArgs>)invocationList[num]).BeginInvoke(null, new RemoteDownloadStatusEventArgs(e.FileUrl, e.Status, e), null, null);
			}
		}
