...
public class DownloadWorker
{
	...
	public DownloadWorker(DownloadInfo info, Action<DownloadInfo> action, Action changePathSuccessAction)
	{
		...
		if (string.IsNullOrEmpty(info.LocalPath))
		{
			...
			if (!Directory.Exists(info.LocalPath))
			{
	...
	public async void Start()
	{
		...
		{
			MoveFile(isMoveZip: true);
			if (status == DownloadStatus.UNENOUGHDISKSPACE)
			{
				FireStatusChangedCallback(DownloadStatus.UNENOUGHDISKSPACE);
				Running = false;
			}
			else if (!ExistsCheck())
			{
	...
	private void MoveFile(bool isMoveZip = false)
	{
		...
		if (isMoveZip)
		{
			string text2 = Path.Combine(Info.LocalPath, Info.FileName);
			string text3 = Path.Combine(text, Info.FileName);
			if (File.Exists(text2))
			{
				...
				catch (Exception ex)
				{
					...
					{
						LogHelper.LogInstance.Info("Move zip failed, msg:" + ex.Message);
					}
					return;
				}
			}
		...
		else
		{
			...
			string text4 = Path.Combine(text, Info.FileName + ".tmp");
			if (File.Exists(TempPath))
			{
				try
				{
					LogHelper.LogInstance.Info("The space required for this decompression is insufficient. If the path has been modified, we will attempt to move the file to the new path");
					GlobalFun.TryDeleteFile(text4);
					File.Move(TempPath, text4);
				}
				catch (Exception)
				{
				}
		...
		status = DownloadStatus.WAITTING;
	}
	...
	{
		string path = Path.Combine(Info.LocalPath, Info.FileName);
		bool flag = GlobalFun.Exists(path);
		if (Info.UnZip && Info.FileType != "TOOL")
		{
			if (flag)
			{
				if (!UnZip(unzip: false))
				{
					flag = false;
		...
		{
			if (string.IsNullOrEmpty(Info.ZipPwd) && Info.FileType == "TOOL")
			{
	...
	private bool UnZip(bool unzip)
	{
		...
		bool result = true;
		int num = Rsd.Instance.Unzip(Info, unzip);
		switch (num)
		{
		case 0:
		...
		default:
			...
			result = false;
			if (!GlobalFun.TryDeleteFile(Path.Combine(Info.LocalPath, Info.FileName)) && num == -1)
			{
				result = true;
			}
			break;
	...
	private void ReadCallback(IAsyncResult asyncResult)
	{
		...
		try
		{
			...
			if (num > 0)
			{
				...
				if (!httpWebRequestState.WriteDoneEvent.WaitOne(10000))
				{
					throw new Exception("byte stream writing to file timeout, timeout period 10000 milliseconds");
				}
