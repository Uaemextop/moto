        ...
        try
        {
            ...
            if (!process.WaitForExit(timeout))
            {
                for (; num < 4; num++)
                {
                    ...
                    string[] outputSnapshot = source.Where(p => !p.ToLower().Contains("waiting for")).ToArray();
                    string[] errorSnapshot = source2.Where(p => !p.ToLower().Contains("waiting for")).ToArray();
                    ...
                    {
                        flag = keyword.Any(item => outputSnapshot.Any(p => p.Contains(item))) || keyword.Any(item => errorSnapshot.Any(p => p.Contains(item)));
                    }
                    if (flag)
                    {
